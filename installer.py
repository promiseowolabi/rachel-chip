...
def install_kalite():
	...
	sudo("printf '\nyes\nyes\nno\n' | sudo kalite manage setup --username=rachel --password=rachel --hostname=itsybrain --description=itsybrain")
...
def wifi_present():
	if is_vagrant():
		return False
	return exists("/sys/class/net/wlan1")
...
if basedir() == "/tmp/rachel_installer":
	...
	sudo("git clone --depth 1 https://promiseowolabi@bitbucket.org/promiseowolabi/rachelchip.git /tmp/rachel_installer") or die("Unable to clone RACHEL installer repository.")
...
if wifi_present() and args.install_wifi:
	...
	sudo("iptables -t nat -A POSTROUTING -o wlan0 -j MASQUERADE") or die("Unable to set iptables MASQUERADE on wlan0.")
	sudo("iptables -A FORWARD -i wlan0 -o wlan1 -m state --state RELATED,ESTABLISHED -j ACCEPT") or die("Unable to forward wlan1 to wlan0.")
	sudo("iptables -A FORWARD -i wlan1 -o wlan0 -j ACCEPT") or die("Unable to forward wlan1 to wlan0.")
	sudo("sh -c 'iptables-save > /etc/iptables.ipv4.nat'") or die("Unable to save iptables configuration.")
	sudo("ifconfig wlan1 10.10.10.10") or die("Unable to set wlan1 IP address (10.10.10.10)")
